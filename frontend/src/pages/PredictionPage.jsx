...
const PredictionResultCard = ({ prediction, explanation, onReset }) => {
  ...
  const handleDownloadPDF = () => {
    const shapRows = top5.map(f => `
      <tr>
        <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;color:#475569">${f.feature_name}</td>
        <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;font-weight:600;color:#0f172a">${f.value}</td>
        <td style="padding:10px 8px;border-bottom:1px solid #f1f5f9;font-weight:600;color:${f.direction === 'higher_risk' ? '#e11d48' : '#16a34a'}">
          ${f.direction === 'higher_risk' ? '↑ Higher risk' : '↓ Lower risk'} (${f.shap_value.toFixed(4)})
        </td>
      </tr>`).join('');

    const shapSection = top5.length > 0 ? `
      <div style="margin-top:32px">
        <h3 style="border-bottom:2px solid #e2e8f0;padding-bottom:10px;color:#0f172a;font-size:18px;margin-bottom:15px">Top Influencing Factors (SHAP)</h3>
        <p style="color:#64748b;font-size:13px;margin-bottom:12px">The strongest factors influencing this model prediction were:</p>
        <table style="width:100%;border-collapse:collapse;text-align:left">
          <thead><tr>
            <th style="padding:8px;background:#f8fafc;color:#64748b;font-size:12px;text-transform:uppercase">Feature</th>
            <th style="padding:8px;background:#f8fafc;color:#64748b;font-size:12px;text-transform:uppercase">Value</th>
            <th style="padding:8px;background:#f8fafc;color:#64748b;font-size:12px;text-transform:uppercase">Contribution</th>
          </tr></thead>
          <tbody>${shapRows}</tbody>
        </table>
        <p style="margin-top:16px;font-size:11px;color:#92400e;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:12px">
          SHAP values explain the behaviour of the machine-learning model for this prediction. They do not establish medical causation and should not be interpreted as a medical diagnosis.
        </p>
      </div>` : '';
    ...
    container.innerHTML = `
      <div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#1e293b;padding:50px;max-width:800px;margin:0 auto;background:white">
        <div style="border-bottom:3px solid ${isHighRisk ? '#e11d48' : '#16a34a'};padding-bottom:20px;margin-bottom:30px">
          ...
          <p style="color:#64748b;margin:0;font-size:14px;text-transform:uppercase;letter-spacing:1px">Cardiac Risk Prediction Report — ${new Date().toLocaleDateString()}</p>
        </div>
        <div style="margin-bottom:32px;padding:24px;border-radius:12px;background:${isHighRisk ? '#fff1f2' : '#f0fdf4'};border:1px solid ${isHighRisk ? '#fecdd3' : '#bbf7d0'};text-align:center">
          <h2 style="margin:0 0 8px;color:${isHighRisk ? '#be123c' : '#15803d'};font-size:28px;font-weight:800">${isHighRisk ? 'HIGH RISK IDENTIFIED' : 'LOW RISK IDENTIFIED'}</h2>
          <p style="margin:0;color:${isHighRisk ? '#9f1239' : '#166534'};font-size:15px">${isHighRisk ? 'Heart disease indicators detected — immediate medical attention recommended.' : 'No significant cardiac risk factors detected based on provided parameters.'}</p>
        </div>
        ${shapSection}
        <div style="margin-top:60px;font-size:11px;color:#94a3b8;text-align:center;border-top:1px solid #e2e8f0;padding-top:20px;line-height:1.5">
          <p style="margin:0 0 5px"><strong>DISCLAIMER:</strong> This AI prediction is for educational and informational purposes only.</p>
          <p style="margin:0 0 5px">It does not replace professional medical advice, diagnosis, or treatment.</p>
  ...
  return (
    <div className="animate-bounce-in mt-8 space-y-4">
      ...
      {/* SHAP Explanation — rendered outside the captured card */}
      <ShapExplanation explanation={explanation} topN={5} />
    </div>
  );
};
  ...
  return (
    <div className="flex-1 py-12 bg-slate-50 dark:bg-dark-950">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Result or Form */}
        {result !== null ? (
          <PredictionResultCard prediction={result} explanation={explanation} onReset={handleReset} />
        ) : (
